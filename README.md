...
npm run import:eopi-knowledge -- --dry-run  # 預覽 EOPI RMA 客服知識庫匯入
...
`scripts/import-eopi-email-knowledge.mjs` 可將 `EOPI RMA Refine/knowledge` 的 Markdown 知識庫匯入 `email_knowledge_sources`，並使用與檔案上傳功能相同的段落優先 chunking（約 1500 字元）建立多筆可檢索來源。

```bash
# 先預覽，不寫入資料庫
npm run import:eopi-knowledge -- --dry-run

# 匯入預設路徑 ../EOPI RMA Refine/knowledge
SUPABASE_URL=... SUPABASE_SERVICE_ROLE_KEY=... npm run import:eopi-knowledge

# 取代已匯入過的同 source_path 內容，並立即啟動 embedding job
SUPABASE_URL=... SUPABASE_SERVICE_ROLE_KEY=... npm run import:eopi-knowledge -- --replace --kickoff

# 指定其他知識庫路徑
npm run import:eopi-knowledge -- --knowledge-dir "/absolute/path/to/knowledge" --dry-run
```

匯入器會依來源檔自動設定 `source_type` 與 `metadata.tag`，例如 `faq.md` → `faq`、`templates.md`/`signature.md` → `template`、`style_examples.md` → `email`、`products/*.md` → `document`。`Reference/` 內的 zip/csv/json/PDF 不會由此腳本自動匯入。
