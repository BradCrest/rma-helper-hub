...
const FollowUpCareEmail = ({
  ...
  <Html lang="zh-Hant" dir="ltr">
    <Head />
    <Preview>關於您 RMA {rmaNumber} 的後續關懷</Preview>
    <Body style={main}>
      <Container style={container}>
        <Heading style={h1}>後續關懷</Heading>
        ...
        {(rmaNumber || productModel) ? (
          <Section style={infoBox}>
            {rmaNumber ? (
              <Text style={infoLine}>RMA 編號：<strong>{rmaNumber}</strong></Text>
            ) : null}
            {productModel ? (
              <Text style={infoLine}>產品型號：<strong>{productModel}</strong></Text>
            ) : null}
          </Section>
        ) : null}
        ...
        {surveyUrl ? (
          <Section style={surveySection}>
            <Text style={surveyTitle}>📝 滿意度問卷（30 秒）</Text>
            <Text style={text}>
              為了持續提升服務品質，懇請您撥冗填寫簡短的滿意度問卷：
            </Text>
        ...
        <Hr style={hr} />
        <Text style={notice}>
          此信件由系統自動寄出，請勿直接回覆此 Email。如有任何疑問，請來信至本公司客服信箱。
        </Text>
...
export const template = {
  ...
  subject: (data: Record<string, any>) =>
    (data?.subject as string) || `${SITE_NAME} — 關於您 RMA ${data?.rmaNumber ?? ''} 的後續關懷`,
  displayName: '客戶關懷信',
  previewData: {
    customerName: '王小明',
